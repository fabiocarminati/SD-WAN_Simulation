function checkCurrentLinkWaves()
    ...
    link1Status=round(link1Status);
    link2Status=round(link2Status);
    if(link1Status>link1MaxThreshold)
        checkLink1=false;    
    end

    %% Check current path and correspondent status
    if(checkLink1==false) 
        if(ControllerLink1Status==1 && CPE1link1Status==1 && CPE2link1Status==1)
            ...
            if(ControllerLink2Status==1 && CPE1link2Status==1 && CPE2link2Status==1)                
                if(link2Status==link1MaxThreshold) 
                    currentLink=2;
